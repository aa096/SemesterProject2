...
const container1 = document.querySelector("#last-container");

export function listingsTemplateLast(item) {
    const divWrapper = document.createElement("div");
    divWrapper.classList.add("text-center", "text-secondary");

    const listingsCard = document.createElement("div");
    listingsCard.classList.add("text-center", "indexp", "my-3");
    ...
    if (item.media && item.media.length > 0) {
        ...
        image.classList.add("card-img-top", "object-fit-cover", "mb-3");
...
export function renderListingsTemplatesLast(items) {
    ...
    dataList
        ...
        .forEach((item) => {
            container1.appendChild(listingsTemplateLast(item));
        });
}
